...
def clean_wallet_database():
    ...
    try:
        with shelve.open("wallet_data") as wallet_data:
            ...
            wallet_keys = []
            for key in all_keys:
                if any(key.startswith(prefix) for prefix in ['public_key', 'private_key', 'nonce_key', 'salt_key']):
...
def verify_cleanup():
    ...
    try:
        with shelve.open("wallet_data") as wallet_data:
            all_keys = list(wallet_data.keys())
            wallet_keys = [key for key in all_keys if any(key.startswith(prefix) for prefix in ['public_key', 'private_key', 'nonce_key', 'salt_key'])]
